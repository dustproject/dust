...
export function createSocket({
  ...
  async function open() {
    debug("open(): calling connect");
    const socket = connect();
    debug("open(): binding open listener/promise");
    const isOpen = waitForOpen(socket);

    debug("open(): binding listeners");
    ...
    debug("open(): waiting for open");
    await isOpen;

    debug("open(): returning socket");
    return socket;
  }
  ...
  async function getSocket() {
    if (socketPromise) {
      if (closed) {
        debug("getSocket(): socket was closed, returning socket");
        return socketPromise;
      }
      debug("getSocket(): have socket");

      const currentSocket = socketPromise;
      debug("getSocket(): awaiting socket");
      ...
      if (socket.readyState !== socket.OPEN) {
        debug("getSocket(): socket not open, unsetting and trying again...");
        socketPromise = null;
        return getSocket();
      }
    }
    ...
    if (!socketPromise) {
      debug("getSocket(): no socket, opening one");
      socketPromise = retry(open);
      const currentSocket = socketPromise;
      debug("getSocket(): awaiting socket");
      const socket = await currentSocket;
      debug("getSocket(): binding listeners");
      socket.addEventListener("error", (event) => {
        debug("socket error, closing and getting new one...");
        // try {
        //   socket.close();
        // } catch {}
        if (currentSocket === socketPromise) {
      ...
      socket.addEventListener("close", (event) => {
        debug("socket close, getting new one...");
        if (currentSocket === socketPromise) {
          getSocket();
        }
      });
...
async function waitForOpen(socket: WebSocket): Promise<WebSocket> {
  debug("waitForOpen(): called");
  switch (socket.readyState) {
    case socket.OPEN:
      debug("waitForOpen(): socket is open, returning");
      return socket;
    case socket.CLOSING:
      debug("waitForOpen(): socket is closing, throwing");
      throw new Error("WebSocket closing.");
    case socket.CLOSED:
      debug("waitForOpen(): socket is closed, throwing");
      throw new Error("WebSocket closed.");
    case socket.CONNECTING:
      debug("waitForOpen(): socket is connecting, adding listener");
      ...
      debug("waitForOpen(): open, returning");
      return socket;
    default:
