...
describe("moveUtils", () => {
  describe("packDirections", () => {
    it("should pack simple directions correctly", () => {
      ...
      expect(Number(packed & mask)).toBe(
        config.enums.Direction.indexOf("PositiveX"),
      );
      expect(Number((packed >> BigInt(5)) & mask)).toBe(
        config.enums.Direction.indexOf("PositiveZ"),
      );
      expect(Number((packed >> BigInt(10)) & mask)).toBe(
        config.enums.Direction.indexOf("NegativeX"),
      );
    });
    ...
    it("should throw error for too many directions", () => {
      ...
      expect(() => packDirections(directions)).toThrow(
        "Too many directions: maximum 50 allowed",
      );
    });
  ...
  describe("unpackDirections", () => {
    it("should unpack directions correctly", () => {
      const originalDirections: Direction[] = [
        "PositiveX",
        "PositiveZ",
        "NegativeX",
      ];
