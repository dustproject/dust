...
library NatureSystemLib {
  ...
  function respawnResource(NatureSystemType self, uint256 blockNumber, ObjectType objectType) internal {
  ...
  function respawnResource(CallWrapper memory self, uint256 blockNumber, ObjectType objectType) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert NatureSystemLib_CallingFromRootSystem();
  ...
  function respawnResource(RootCallWrapper memory self, uint256 blockNumber, ObjectType objectType) internal {
    bytes memory systemCall = abi.encodeCall(
      _respawnResource_uint256_ObjectType.respawnResource,
      (blockNumber, objectType)
    );
