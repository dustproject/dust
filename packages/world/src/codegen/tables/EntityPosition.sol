...
library EntityPosition {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "", name: "EntityPosition", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74620000000000000000000000000000456e74697479506f736974696f6e0000);
   ...
   */
  function get(EntityId entityId) internal view returns (EntityPositionData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = EntityId.unwrap(entityId);
   ...
   * @notice Get the full data.
   */
  function _get(EntityId entityId) internal view returns (EntityPositionData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = EntityId.unwrap(entityId);
   ...
   */
  function set(EntityId entityId, EntityPositionData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(EntityId entityId, EntityPositionData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.z);
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (EntityPositionData memory _table) {
