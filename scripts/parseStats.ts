...
import { type Hex, formatEther, parseAbi } from "viem";
...
async function main() {
  ...
  const userNamesData = await fetch(
    "https://biome1.biomes.aw/api/user/names",
  ).then((res) => res.json());
  ...
  for (const user in allUserStats) {
    ...
    const txCounts = allUserStats[user].txCounts;
    let firstTxDate = allUserStats[user].firstTxDate
      ? new Date(allUserStats[user].firstTxDate)
      : null;
    let lastTxDate = allUserStats[user].lastTxDate
      ? new Date(allUserStats[user].lastTxDate)
      : null;
    ...
    if (delegator) {
      ...
      if (delegatorStats !== undefined) {
        ...
        if (
          firstTxDate === null ||
          (delegatorStats.firstTxDate !== null &&
            new Date(delegatorStats.firstTxDate) < firstTxDate)
        ) {
          firstTxDate = new Date(delegatorStats.firstTxDate);
        }
        if (
          lastTxDate === null ||
          (delegatorStats.lastTxDate !== null &&
            new Date(delegatorStats.lastTxDate) > lastTxDate)
        ) {
    ...
    if (delegatee) {
      ...
      if (delegateeStats !== undefined) {
        ...
        if (
          firstTxDate === null ||
          (delegateeStats.firstTxDate !== null &&
            new Date(delegateeStats.firstTxDate) < firstTxDate)
        ) {
          firstTxDate = new Date(delegateeStats.firstTxDate);
        }
        if (
          lastTxDate === null ||
          (delegateeStats.lastTxDate !== null &&
            new Date(delegateeStats.lastTxDate) > lastTxDate)
        ) {
    ...
    fnNameCsvRows.push([
      userAddress,
      username,
      ...fnNameArr.map((fn) => txCounts.get(fn) ?? 0),
    ]);
  }
  ...
  fs.writeFileSync(
    "gen/userStatsFn.csv",
    fnNameCsvRows.map((row) => row.join(",")).join("\n"),
  );
}
